...
public class ClassRandomForestOptimizer {
    ...
    private ArrayList<double[]> performanceMetrics; // Combination of performance metrics (accuracy, errors)
    private Random random = new Random();
    
    public ClassRandomForestOptimizer(RandomForest model, DataFrame dataFrame, int maxRetries) {
        this.model = model;
    ...
    private RandomForest optimizeModel(RandomForest model, int maxRetries, int retries) {
        if (retries >= maxRetries || isModelAcceptable(model)) {
            System.out.println(retries + " " + model.metrics().accuracy());
            return model;
        }
        ...
        switch (retries) {
            case 0: // the first retry
                ...
                newModel = RandomForest.fit(Formula.lhs("RankBracket"), dataFrame, maxOptions);
                break;
            case 1: // the second retry
                ...
                newModel = RandomForest.fit(Formula.lhs("RankBracket"), dataFrame, minOptions);
                break;
    ...
    private boolean isModelAcceptable(RandomForest model) {
        return model.metrics().accuracy() > 0.8 && model.metrics().error() < 200;
    }

    private double[] getMetrics(RandomForest model) {
        double[] metrics = new double[2];
        metrics[0] = model.metrics().accuracy();
        metrics[1] = model.metrics().error();
        return metrics;
    }
    ...
    private RandomForest createNewRandomForest() {
        ...
        RandomForest.Options newOptions = new RandomForest.Options(
            (int)bestCandidate[0], (int)bestCandidate[1], (int)bestCandidate[2], (int)bestCandidate[3], (int)bestCandidate[4]);
        return RandomForest.fit(Formula.lhs("RankBracket"), dataFrame, newOptions); 
    }
